...
function bingo_check(){
    ...
    for(var i=0; i<Brow; i++){ // 줄
        for(var k=0; k<Bcol; k++){ // 칸
            if(bingo[i*5+k] == 0) // 가로방향으로 한 줄씩 0이 몇개인가 확인
            row++; // 한 칸에 0이 있을때 마다 1씩 증가
            if(bingo[k*5+i] == 0) // 세로방향으로 한줄씩 0이 몇개인가 확인
            col++; // 세로 방향 한칸에 0이 5개라면 1줄 빙고
        }
        ...
        if(col==5) end++; // 세로방향 한 줄에 0이 5개 라면 1줄 빙고
        row=0;
        col=0;
    }
...
function state_check(){
    ...
    this.classList.toggle("st_check"); // 있으면 클래스 삭제
    else
    this.classList.add("st_check"); // 없으면 클래스 추가
